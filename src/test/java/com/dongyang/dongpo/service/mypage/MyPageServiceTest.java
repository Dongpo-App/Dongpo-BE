...
class MyPageServiceTest {
    ...
    void getMyRegisteredStores() {
        // given
        Member member = mock(Member.class);
        StoreSummaryResponseDto store1 = mock(StoreSummaryResponseDto.class);
        StoreSummaryResponseDto store2 = mock(StoreSummaryResponseDto.class);
        List<StoreSummaryResponseDto> expectedStores = List.of(store1, store2);

        when(storeService.getMyRegisteredStores(member)).thenReturn(expectedStores);

        // when
        List<StoreSummaryResponseDto> storeSummaryResponseDtos = myPageService.getMyRegisteredStores(member);

        // then
        assertThat(storeSummaryResponseDtos).isEqualTo(expectedStores);
