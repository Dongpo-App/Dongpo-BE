...
@Transactional
public class StoreVisitCertService {

    private final StoreVisitCertRepository storeVisitCertRepository;
    private final StoreUtil storeUtil;

    // 방문인증 내역 추가
    protected void addStoreVisitCert(final Store store, final Member member, final Boolean isVisitSuccessful) {
    ...
    protected Long countVisitCertSuccessByStoreAndTimeRangeAndDayOfWeek(final Store store, final TimeRange timeRange, final DayOfWeek dayOfWeek) {
    ...
    protected Boolean checkStoreVisitCertBy24Hours(final Store store, final Member member) {
        return storeVisitCertRepository.existsByStoreAndMemberWithin24Hours(store, member, LocalDateTime.now().minusHours(24));
    }

    // 사용자의 방문 인증 성공 횟수 반환
    protected Long getStoreVisitCertSuccessCount(final Member member) {
        return storeVisitCertRepository.countByMemberAndIsVisitSuccessfulIsTrue(member);
    }

    // 사용자의 방문 인증 실패 횟수 반환
    protected Long getStoreVisitCertFailCount(final Member member) {
